...
$SD.on('didReceiveGlobalSettings', (jsn) => {
    console.log("didReceiveGlobalSettings", jsn);
    updateUI(jsn.payload.settings)
});
...
const updateUI = (pl) => {
    if (pl['g_cutoff_1'] !== undefined) {
        showAsGood();
    }
    if (pl['b_cutoff_1'] !== undefined) {
        showAsBad();
    }
    Object.keys(pl).map(e => {
        if (e && e !== '' && e !== 'good_selected' && e !== 'bad_selected') {
...
$SD.on('piDataChanged', (returnValue) => {
    ...
    else if (returnValue.key === 'apiToken') {
        $SD.api.setGlobalSettings($SD.uuid, {apiToken: returnValue.value });
    }
    else {
...
function showAsGood() {
    document.getElementById('color_settings_good').hidden = false;
    document.getElementById('color_settings_bad').hidden = true;

    document.getElementById('good_selected').checked = true;
}
function showAsBad() {
    document.getElementById('color_settings_bad').hidden = false;
    document.getElementById('color_settings_good').hidden = true;

    document.getElementById('bad_selected').checked = true;
}
